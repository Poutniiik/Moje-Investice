...
@st.cache_data(ttl=3600) # 1 hodina cache pro kurzy (used in portfolio service via import but cache decorator is local there or here?)
# The cache decorators are on functions. If I import them, they are cached.
# But some functions were defined in web_investice.py with decorators.
# I moved them to portfolio_service.py with decorators.
...
def main():
    ...
    if 'df' not in st.session_state:
        with st.spinner("NAČÍTÁM DATA..."):
            ...
            # We import aktualizuj_graf_vyvoje from portfolio_service, but we need to check if it's available or we can just initialize it manually
    ...
    # Import cached_kurzy from somewhere or define it.
    # It was in web_investice.py. We can move it to utils or portfolio_service.
    # It's better to use the one from portfolio_service if available or utils.
