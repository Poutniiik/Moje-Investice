...
from src.utils import (
    make_plotly_cyberpunk, ziskej_detail_akcie, ziskej_earnings_datum, calculate_sharpe_ratio,
    ziskej_historicka_data, ziskej_benchmark_data, ziskej_data_pro_predikci, ziskej_dca_data
)
...
def render_analýza_rentgen_page(df, df_watch, vdf, model, AI_AVAILABLE):
    ...
    if vybrana_akcie:
        with st.spinner(f"Načítám rentgen pro {vybrana_akcie}..."):
            # PŮVODNÍ: t_info, hist_data = ziskej_detail_akcie(vybrana_akcie)
            # OPTIMALIZACE:
            t_info, hist_data = cached_detail_akcie(vybrana_akcie)
        ...
        if len(tickers_list) > 1:
            try:
                with st.spinner("Počítám korelace..."):
                    # PŮVODNÍ (BEZ CACHE): hist_data = yf.download(tickers_list, period="1y")['Close']
                    # OPTIMALIZACE:
                    hist_data = ziskej_historicka_data(tickers_list, period="1y")
...
def render_analýza_page(df, df_watch, vdf, model, AI_AVAILABLE, kurzy, celk_hod_usd, hist_vyvoje, viz_data_list, celk_hod_czk, LIVE_DATA):
        ...
        if tickers_to_compare:
            try:
                with st.spinner(f"Stahuji historická data pro {len(tickers_to_compare)} tickerů..."):
                    # PŮVODNÍ: raw_data = yf.download(tickers_to_compare, period="1y", interval="1d", progress=False)['Close']
                    # OPTIMALIZACE:
                    raw_data = ziskej_historicka_data(tickers_to_compare, period="1y", interval="1d")
                ...
                if raw_data.empty:
                    ...
                    for t in tickers_to_compare[:4]:
                        # PŮVODNÍ: i, h = cached_detail_akcie(t)
                        # OPTIMALIZACE:
            ...
            if st.button("🧠 AKTIVOVAT NEURONOVOU SÍŤ", type="primary"):
                try:
                    ...
                    with st.spinner(f"Trénuji model na datech {pred_ticker}..."):
                        # PŮVODNÍ: hist_train = yf.download(pred_ticker, period="2y", progress=False)
                        # OPTIMALIZACE:
                        hist_train = ziskej_data_pro_predikci(pred_ticker, period="2y")
            ...
            if st.button("🚀 SPUSTIT SIMULACI", key="btn_dca"):
                with st.spinner("Počítám..."):
                    try:
                        ...
                        # PŮVODNÍ: hist = yf.download(dca_ticker, start=start, interval="1mo", progress=False)['Close']
                        # OPTIMALIZACE:
                        hist = ziskej_dca_data(dca_ticker, start, interval="1mo")
            ...
            if len(tickers_ef) < 2:
                ...
                if st.button("📈 Vypočítat optimální portfolio"):
                    with st.spinner("Simuluji 5000 portfolií..."):
                        try:
                            # PŮVODNÍ: data = yf.download(tickers_ef, period="2y", progress=False)['Close']
                            # OPTIMALIZACE:
                            data = ziskej_historicka_data(tickers_ef, period="2y")
    ...
    with tab5:
        ...
        if not hist_vyvoje.empty and len(hist_vyvoje) > 1:
            ...
            try:
                # PŮVODNÍ: sp500 = yf.download("^GSPC", start=start_date, progress=False)
                # OPTIMALIZACE:
                sp500 = ziskej_benchmark_data("^GSPC", start_date)
