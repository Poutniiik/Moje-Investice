    ...
    if page == "🏠 Přehled":
        ...
        with tab7:
            ...
            if not vdf.empty:
                ...
                if total_target != 100:
                    st.warning(f"⚠️ Součet cílů je {total_target}%. Měl by být 100%.")
                ...
                for _, r in df_reb.iterrows():
                    diff = r['Rozdíl']
                    curr = "USD" # Calculations in USD
                    if abs(diff) > 1: # Ignore small dust
                        if diff > 0:
                            st.success(f"🟢 **{r['Sektor']}**: DOKOUPIT za {diff:,.0f} $")
                        else:
                            st.error(f"🔴 **{r['Sektor']}**: PRODAT za {abs(diff):,.0f} $")
