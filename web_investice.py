...
# ----------------------------------------------------------------------
# VÝVOJOVÝ TIP: PŘESUNUTÍ LOGIKY STRÁNEK DO VLASTNÍCH FUNKCÍ
# Tím získáme čistější a bezpečnější hlavní funkci 'main'.
# ----------------------------------------------------------------------
...
def main():
    ...
    def process_cli_command():
        ...
        try:
            if cmd == "/help":
                ...
                p_cli, m_cli, _ = ziskej_info(t_cli)
                if p_cli:
                    ok, msg = proved_prodej(t_cli, k_cli, p_cli, USER, m)
                    msg_text = msg
                    msg_icon = "✅" if ok else "❌"
                else:
                    msg_text = "❌ Chyba ceny"
    ...
    if page == "🏠 Přehled":
        ...
        # Ponechávám zbytek kódu "Analýza"
        # Kvůli délce ponecháváme zbytek stránek bez refaktoringu, dokud mě nepožádáš o další krok!
        # DŮLEŽITÉ: Následující kód by měl být původní kompletní kód z Analýza a dalších stránek.
