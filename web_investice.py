...
def main():
    ...
    # --- FIX PRO DATA VE VŠECH STRÁNKÁCH ---
    # Vytvoříme vdf (tabulku dat) hned zde, aby byla dostupná všude
    if not df.empty:
            ...
            if not hist_vyvoje.empty and len(hist_vyvoje) > 1:
                ...
                try:
                    ...
                    if not sp500.empty:
                        ...
                        c_b1.metric("Můj výnos", f"{my_last:+.2f} %"); c_b2.metric("S&P 500", f"{sp_last:+.2f} %"); c_b3.metric("Rozdíl", f"{diff:+.2f} %", delta=diff)
                    ...
                    try:
                        with st.spinner("Počítám korelace..."):
                            ...
                            if avg_corr > 0.7:
                                ...
                                st.warning("⚖️ Střední korelace. Portfolio je vyvážené, ale sleduj ho.")
